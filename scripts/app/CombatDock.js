...
export class CombatDock extends HandlebarsApplication {
    ...
    get sortedCombatants() {
        const combatants = Array.from(this.combat.combatants.contents).sort(this.combat._sortCombatants);
        if (!this.isDaggerheart) return combatants;
        ...
        for (const combatant of combatants) {
            if (isDaggerheartCharacter(combatant)) {
                if (isDaggerheartSpotlightRequesting(combatant)) spotlightRequests.push(combatant);
                else characters.push(combatant);
            } else {
                adversaries.push(combatant);
            }
        }

        spotlightRequests.sort((a, b) => (a.system?.spotlight?.requestOrderIndex ?? 0) - (b.system?.spotlight?.requestOrderIndex ?? 0));

        return [...spotlightRequests, ...characters, ...adversaries];
    }
    ...
    get currentCombatant() {
        return this.combat.combatant ?? this.sortedCombatants[0] ?? null;
    }

    get trueCarousel() {
        return !this.isDaggerheart && game.settings.get(MODULE_ID, "carouselStyle") < 2;
    }
    ...
    setupSeparator(){
        if (!this.trueCarousel || !this.element) return;
        const combatantsContainer = this.element.querySelector("#combatants");
    ...
    updateCombatant(combatant, updates = {}) {
        const updateKeys = Object.keys(foundry.utils.flattenObject(updates));
        const spotlightChanged =
            this.isDaggerheart &&
            (
                foundry.utils.hasProperty(updates, "system.spotlight") ||
                foundry.utils.hasProperty(updates, "system.spotlight.requesting") ||
                foundry.utils.hasProperty(updates, "system.spotlight.requestOrderIndex") ||
    ...
    updateOrder() {
        ...
        if (separator) separator.style.display = isTrueCarousel ? "" : "none";
        ...
        const currentCombatant = this.currentCombatant;
    ...
    centerCurrentCombatant() {
        ...
        if (this.trueCarousel) {
            ...
            const current = this.portraits.find((p) => p.combatant === this.currentCombatant);
            if (!current) return;
